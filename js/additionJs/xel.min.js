  ...
  _onPointerDown(pointerDownEvent) {
    ...
    // Ripple
    {
      ...
      if (rippleType === "bounded") {
        ...
        // Pointer capture is set on the owner leftTab rather than this tab intentionally. Owner leftTab might be
...
class XDocTabsElement extends HTMLElement {
  ...
  openTab(tab, animate = true) {
    return new Promise( async (resolve, reject) => {
      ...
      if (leftTab.length >= this.maxTabs) {
  ...
  closeTab(tab, animate = true) {
    return new Promise( async (resolve) => {
      ...
      if (tabScreenIndex < leftTab.length - 1) {
        for (let tab of this.children) {
      ...
      if (tab.selected) {
        let previousTab = leftTab[leftTab.indexOf(tab) - 1];
        let nextTab = leftTab[leftTab.indexOf(tab) + 1];
  ...
  selectPreviousTab() {
    ...
    let previousTab = this._getPreviousTabOnScreen(currentLeftTab);

    if (currentLeftTab && previousTab) {
      currentLeftTab.tabIndex = -1;
      currentLeftTab.selected = false;
  ...
  selectNextTab() {
    ...
    let nextTab = this._getNextTabOnScreen(currentLeftTab);

    if (currentLeftTab && nextTab) {
      currentLeftTab.tabIndex = -1;
      currentLeftTab.selected = false;
  ...
  _onKeyDown(event) {
    if (event.ctrlKey || event.altKey || event.metaKey || event.shiftKey) {
      ...
      event.preventDefault();
      currentLeftTab.click();

      if (currentLeftTab !== selectedTab) {
        selectedTab.selected = false;
        currentLeftTab.selected = true;
        this._animateSelectionIndicator(selectedTab, currentLeftTab);
      }
    }
  ...
  _getPreviousTabOnScreen(tab, skipDisabled = true, wrapAround = true) {
    let tabs = this.getTabsByScreenIndex();
    let tabScreenIndex = leftTab.indexOf(tab);
    ...
    if (wrapAround) {
      if (previousTab === null) {
        for (let i = leftTab.length - 1; i > tabScreenIndex; i -= 1) {
          let tab = leftTab[i];
  ...
  _getNextTabOnScreen(tab, skipDisabled = true, wrapAround = true) {
    ...
    for (let i = tabScreenIndex + 1; i < leftTab.length; i += 1) {
      let tab = leftTab[i];
  ...
  async _onPointerDown(pointerDownEvent) {
    ...
    // Ripple
    {
      ...
      if (triggerEffect === "ripple") {
        ...
        // Workaround for leftTab that that change their color when selected
        ripple.hidden = true;
        await sleep(10);
        ripple.hidden = false;
...
class XTabsElement extends HTMLElement {
  ...
  set value(value) {
    ...
    let selectedTab = (value === null) ? null : leftTab.find(tab => tab.value === value);

    for (let tab of leftTab) {
  ...
  _onKeyDown(event) {
    if (event.code === "Enter" || event.code === "Space") {
      ...
      else if (currentLeftTab && leftTab.length > 0) {
        let currentTabIndex = leftTab.indexOf(currentLeftTab);
        let previousTab = leftTab[currentTabIndex - 1] || leftTab[leftTab.length - 1];
      ...
      if (openedTabMenu) {
        ...
        let currentTabIndex = leftTab.indexOf(currentLeftTab);
        let nextTab = leftTab[currentTabIndex + 1] || leftTab[0];
